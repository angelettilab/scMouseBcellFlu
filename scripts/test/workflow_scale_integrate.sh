...
# ###################################################
# mkdir $main/'analysis/02_cluster_scale'
...
# 2>&1 | tee $main/'analysis/02_cluster_scale/integrate_log.txt'
...
# --assay 'RNA' \
# --output_path $main/'analysis/03_diff_expr_scale' \
# 2>&1 | tee $main/'analysis/03_diff_expr_scale/diff_expr_log.txt'
...
# --output_path $main/'analysis/04_remove_cells_scale' \
# 2>&1 | tee $main/'analysis/04_remove_cells_scale/remove_cells_log.txt'
...
# --assay 'RNA' \
# --remove_cells $main/'analysis/04_remove_cells_scale/remove_cell_barcodes.txt' \
# --output_path $main/'analysis/05_qc_scale' \
# 2>&1 | tee $main/'analysis/05_qc_scale/load_data_log.txt'
...
# --Seurat_object_path $main/'analysis/05_qc_scale/raw_seurat_object.rds' \
# --columns_metadata $var_to_plot \
...
# --assay 'RNA' \
# --output_path $main/'analysis/05_qc_scale' \
# 2>&1 | tee $main/'analysis/05_qc_scale/qc_log.txt'
...
# ### RUN INTEGRATION WITH PRE-SCALING/REGRESSION ###
# ###################################################
# mkdir $main/'analysis/06_cluster_scale'
# Rscript $main/scripts/02b_scale_integrate.R \
...
# 2>&1 | tee $main/'analysis/06_cluster_scale/integrate_log.txt'
...
--exclude_cluster 'NONE' \
--assay 'RNA' \
--output_path $main/'analysis/07_diff_expr_scale' \
2>&1 | tee $main/'analysis/07_diff_expr_scale/diff_expr_log.txt'
