...
cdir <- paste0(adir,'04_cluster/')  # clustering subdirectory
sdir <- paste0(cdir,'cell_type_prediction/')  # cell type prediction subdirectory
tdirs <- dir(adir, '^trajectory_\\d')  # analysis subdirectories starting with "trajectory_"
...
```{r, results='asis'}
traj_table <- read.csv(paste0(adir, 'trajectory_summary/trajectory_runs_summary.csv'), row.names=1, stringsAsFactors=F)
...
for (t in tdirs) {
  ...
  if (length(fig_name) == 0) {
    ...
    # show figure with only one curve
