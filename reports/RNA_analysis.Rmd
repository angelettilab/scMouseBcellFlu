...
```{r}
cdir <- paste0(adir, '02_cluster_scale/')
```
...
```{r}
edir <- paste0(adir,'03_diff_expr_scale/')
```
...
```{r, results='asis'}
cat('![](', adir, '04_remove_cells_scale/umap_removed_cells.png', '){width=80%}', sep='')
```
...
```{r}
cdir <- paste0(adir, '06_cluster_scale/')
```
...
### Differential expression {.tabset}
```{r}
edir <- paste0(adir,'07_diff_expr_scale/')
```
